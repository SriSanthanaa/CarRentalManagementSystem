...
/**
 * Servlet implementation class bookServletUser
 */
public class bookServletUser extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public bookServletUser() {
        super();
			...
			if(c != null) {
				...
				if(st >= 0) {
					jakarta.servlet.http.HttpSession session = request.getSession(true);	    
			         ...
			         RequestDispatcher rd = request.getRequestDispatcher("booksuccessuser.jsp");
			         rd.forward(request, response);
				}
				else if(st == -1) {
					response.sendRedirect("reserveduser.jsp");
				}
				else {
